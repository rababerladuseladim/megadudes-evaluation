...
rule download_uniprot_resources:
    output:
        speclist="resources/uniprot/speclist.txt",
    ...
    shell:
        """\
        UNIPROT_VERSION=$(curl --head -sS https://rest.uniprot.org/uniprotkb/ 2>"{log}"| grep x-uniprot-release: | sed 's/x-uniprot-release: //');
